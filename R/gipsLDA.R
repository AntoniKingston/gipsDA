...
gipslda.formula <- function(formula, data, ..., subset, na.action) {
  ...
  m <- eval.parent(m)
  Terms <- attr(m, "terms")
  grouping <- stats::model.response(m)
  ...
  res$xlevels <- stats::.getXlevels(Terms, m)
...
gipslda.matrix <- function(x, grouping, ..., subset, na.action) {
  ...
  if (!missing(na.action)) {
    dfr <- na.action(data.frame(g = grouping, x, check.names = FALSE))
    grouping <- dfr$g
    x <- as.matrix(dfr[, -1L, drop = FALSE])
  }
...
predict.gipslda <- function(object, newdata, prior = object$prior, dimen,
  ...
  if (!is.null(Terms <- object$terms)) { # formula fit
    ...
    x <- stats::model.matrix(Terms, newdata, contrasts.arg = object$contrasts)
    xint <- match("(Intercept)", colnames(x), nomatch = 0L)
    if (xint > 0L) x <- x[, -xint, drop = FALSE]
...
plot.gipslda <- function(x, panel = panel.gipslda, ..., cex = 0.7,
                         dimen, abbrev = FALSE,
                         xlab = "LD1", ylab = "LD2") {
  panel.gipslda <- function(x, y, ...) graphics::text(x, y, as.character(g), cex = cex, ...)
  if (!is.null(Terms <- x$terms)) { # formula fit
    data <- stats::model.frame(x)
    X <- stats::model.matrix(stats::delete.response(Terms), data)
    g <- stats::model.response(data)
    xint <- match("(Intercept)", colnames(X), nomatch = 0L)
    if (xint > 0L) X <- X[, -xint, drop = FALSE]
  } else { # matrix or data-frame fit
    xname <- x$call$x
    ...
    if (!is.null(sub <- x$call$subset)) {
      X <- eval.parent(parse(text = paste(
        deparse(xname, backtick = TRUE),
        "[", deparse(sub, backtick = TRUE), ",]"
      )))
      g <- eval.parent(parse(text = paste(
        deparse(gname, backtick = TRUE),
        "[", deparse(sub, backtick = TRUE), "]"
      )))
    } else {
      X <- eval.parent(xname)
  ...
  if (ncol(X) > 2L) {
    graphics::pairs(X, panel = panel, ...)
  } else if (ncol(X) == 2L) {
    MASS::eqscplot(X[, 1L:2L], xlab = xlab, ylab = ylab, type = "n", ...)
    panel(X[, 1L], X[, 2L], ...)
  } else {
...
ldahist <-
  function(data, g, nbins = 25, h, x0 = -h / 1000, breaks,
    ...
    if (!sep) {
      ...
      oldpar <- graphics::par(mfrow = c(length(groups), 1L))
    ...
    for (grp in groups) {
      ...
      if (type == "histogram" || type == "both") {
        n <- length(breaks)
        graphics::rect(breaks[-n], 0, breaks[-1L], est[[grp]], col = col, ...)
      }
      if (type == "density" || type == "both") graphics::lines(xd[[grp]])
    }
    invisible()
  }
...
pairs.gipslda <- function(x, labels = colnames(x), panel = panel.gipslda,
  ...
  if (type == "std") {
    ...
    print(lattice::splom(~X,
      groups = g, panel = lattice::panel.superpose,
      key = list(
        text = list(levels(g)),
        points = lattice::Rows(
