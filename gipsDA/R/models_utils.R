...
project_cov <- function(emp_cov, n_obs, MAP = TRUE, tol = 1e-3) {
    ...
    gg <- gips::find_MAP(gg, optimizer = "BF",return_probabilities = TRUE, save_all_perms = TRUE)
    probs <- gips::get_probabilities_from_gips(gg)
    probs <- probs[probs > tol]
    return(project_matrix_multiperm(emp_cov, probs))
}

project_covs <- function(emp_covs, ns_obs, MAP = TRUE, tol = 1e-3) {
    ...
    gg <- find_MAP(gg, optimizer = "BF",return_probabilities = TRUE, save_all_perms = TRUE)
